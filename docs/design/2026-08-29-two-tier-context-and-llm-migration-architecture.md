...
**Date:** 2026-08-29 (Updated 2026-08-31; **amended 2026-09-01**, see Amendment 1; **amended 2026-09-21**, see Amendment 3)  
**Status:** Implemented 2026-08-31; §7 amended 2026-09-01 after the skill it specifies shipped contradicting it; **§4, §5 and §7 retired 2026-09-21**  
**Applies to:** `agents` CLI (`init`, `wire`, `doctor`), `AGENTS.md` / `CLAUDE.md`, `.agents/AGENTS.md`, `.agents/skills/`  
...
remedy in §6 named the deleted command `agents update --apply`, an invocation the
CLI then rejected with `--all is required` (both the command and the remedy are
gone — see Amendment 3). Both are corrected above. Neither would have been found by
...
[layout manifest and store-root freedom](../archive/design/2026-09-18-layout-manifest-and-store-root-design.md)
(**archived 2026-09-21: the manifest, the migration and the version floor were all
deleted**, so this amendment no longer describes the tool).
...
## Amendment 3 — 2026-09-21

The reduction that deleted the layout manifest and its migration also deleted
three of this document's subjects, and this is the record of which. **§2 (the
two-tier context architecture) is the reason this document is still the design
in force and is not archived**; so are §3's asset embedding, §6's remaining
`scaffold:*` checks, §8 and §9. What follows is no longer in force:

| § | Was | Now |
|---|---|---|
| 4 | `internal/drift`, the digest catalog of canonical router and skill texts, `DriftReport`, and `agents drift [--json] [--repo] [--all]` | all deleted. Nothing classifies a router or a skill as current, legacy or diverged; there is no currency report and no advisory exit for one |
| 5 | `agents update [--all] [--apply]`, refreshing `agents`-owned infrastructural skills | deleted. Wiring is regenerated by `agents wire`, which does only that; no command refreshes a skill |
| 7 | the `migrating-fleet-context` Model A migration skill — self-currency check, router state table, three-way merge, traceability gate, fleet mode | deleted in full, with the `internal/drift` classifier it drove and the `agents ls` fleet registry it enumerated. The skill text is kept as evidence in [`docs/archive/skills/migrating-fleet-context/`](../archive/skills/migrating-fleet-context/) |
| 8.2 | shadowing native platform planning workflows | unchanged and still in force; listed here only because §7 was its only enforcement |
| 6 | `scaffold:router`, `scaffold:symlink`, `scaffold:domain`, `scaffold:skill-recording` | in force, unchanged. The fifth check, `scaffold:skill-migrating`, is gone with the skill |
| 2, 3 | `.agents/skills/migrating-fleet-context/` in the tree diagram and the asset layout | the binary embeds one skill, `recording-what-you-learn` |
| 10 | the four-phase roadmap | phases 1 is implemented; phases 2–4 were implemented and then deleted by the 2026-09-21 reduction. It is a record of how the system got here, not a plan |

Two claims elsewhere in the body are corrected in place rather than listed here:
§1.2's inventory of the CLI, and §2/§3's asset trees. The reasoning in §4, §5 and
§7 is left exactly as written — it is why the subsystem existed, and it is the
record a future author needs before rebuilding any part of it.

**What replaced the drift check.** Nothing does, and that is deliberate. `agents
doctor` still reports whether the router is present, whether `CLAUDE.md` is the
relative symlink, whether the domain doc exists, and whether the bundled skill
is installed; it no longer reports whether any of them matches a canonical text,
because there is no longer a canonical-text catalog to match against.
...
2. **Deterministic CLI Engine of Truth**:
   ...
   - `agents/internal/drift/`: deleted 2026-09-21 (Amendment 3) — the deterministic package that classified router and skill states (`clean_current`, `clean_legacy`, `customized`, `missing`).
   - `agents drift [--json]`: deleted 2026-09-21 (Amendment 3) — the machine-readable inspection interface emitting structured diagnostics for AI agents.
   - `agents doctor`: Granular diagnostic checks (`scaffold:router`, `scaffold:symlink`, `scaffold:domain`, `scaffold:skill-recording`; the fifth, `scaffold:skill-migrating`, is deleted 2026-09-21 with the skill it guarded).
   - `agents update`: deleted 2026-09-21 (Amendment 3) — it did deterministic machine wiring and authoritative refresh of `agents`-owned infrastructural skills. Wiring is still regenerated, by `agents wire`, which does nothing else.
3. **Model A LLM-in-the-Loop Migration Engine (`migrating-fleet-context`)** — **deleted 2026-09-21 (Amendment 3), as was the fleet registry it enumerated**:
...
The `migrating-fleet-context/` asset this tree used to carry was deleted
2026-09-21 with the migration it described; its text is kept as evidence in
[`docs/archive/skills/migrating-fleet-context/`](../archive/skills/migrating-fleet-context/).
...
The second bundled skill, `migrating-fleet-context/SKILL.md`, was deleted
2026-09-21 and is no longer installed; the step that populated it is removed
above.

---

## 4. Deterministic Drift Inspection Subsystem (`internal/drift`) — retired, deleted 2026-09-21

> **Retired 2026-09-21 (Amendment 3).** `internal/drift` and the `agents drift`
> subcommand are both deleted, and nothing replaced them. The reasoning below is
> kept because it is why the subsystem was built and what a future author needs
> before rebuilding any part of it.

The `internal/drift` package provided pure, deterministic inspection without invoking external processes or network APIs.
...
### 4.3 The `agents drift` Subcommand (retired, deleted 2026-09-21)
...
## 5. Refined `agents update` & Machine Wiring — retired, deleted 2026-09-21

> **Retired 2026-09-21 (Amendment 3).** The `agents update` command is deleted.
> `agents wire` still regenerates harness configs and does nothing else; no
> command refreshes a bundled skill any more.

The `agents update` command (deleted 2026-09-21) had **one focused responsibility**: updating deterministic machine wiring and keeping `agents`-owned infrastructural skills synchronized with the installed binary.

### 5.1 Single-Responsibility Workflow
When running `agents update [--all] [--apply]` (deleted 2026-09-21):
1. **Machine Hook Rewiring (`wire`)**: Synchronizes `.claude/settings.json`, `.codex/hooks.json`, and `.agents/hooks.json`.
2. **Infrastructural Skill Refresh**: Ensures 100% `agents`-owned infrastructural skills (`.agents/skills/migrating-fleet-context/SKILL.md`, itself deleted 2026-09-21) match the embedded version in the running binary.
3. **Safe Router & Skill Boundary**:
   - `agents update` (deleted 2026-09-21) **NEVER** performs regex/string replacement on human domain content (`AGENTS.md`, `.agents/AGENTS.md`) or user-facing customizable skills (`recording-what-you-learn`).
   - If drift or outdated templates are detected, `agents update` (deleted 2026-09-21) emitted an advisory:
     ```text
...
Replaces the legacy generic `scaffold:doctor-instruction` with granular checks. Four of the five below are still in force and are what `agents doctor` reports today; the fifth needed the migration skill and went with it.

| Check Name | Status `ok` | Status `warn` / `fail` / `info` | Remedy Message |
| :--- | :--- | :--- | :--- |
| `scaffold:router` | Present | `info` / `warn` (missing) | Run `agents init` to write the router |
| `scaffold:symlink` | `CLAUDE.md` is relative symlink to `AGENTS.md` | `warn` (missing) / `fail` (a regular file, a wrong target, or a dangling link) | Run `agents init` or recreate symlink |
| `scaffold:domain` | `.agents/AGENTS.md` exists | `warn` (missing) | Run `agents init` or create `.agents/AGENTS.md` |
...
| ~~`scaffold:skill-migrating`~~ | ~~`.agents/skills/migrating-fleet-context/` exists~~ | **deleted 2026-09-21** with the skill; its remedy named `agents update --all --apply`, also deleted | — |

The first row used to read "Matches `CleanCurrent`" and the drift remedy was
"Run `migrating-fleet-context` skill to un-nest domain rules". Both are gone with
the digest catalog. `doctor` now reports whether the router is *present*, which
is all it can check without a canonical text to compare against.

---

## 7. Model A LLM-in-the-Loop Migration Engine (`migrating-fleet-context`) — retired, deleted 2026-09-21

> **Retired 2026-09-21 (Amendment 3).** The skill, the `internal/drift`
> classifier it drove and the deleted `agents ls` fleet registry it enumerated are
> all gone. Every command this section names is deleted. The text is kept as the
> record of what the migration did and why; the skill's own text is in
> [`docs/archive/skills/migrating-fleet-context/`](../archive/skills/migrating-fleet-context/).

The `migrating-fleet-context` skill was an authoritative agent skill embedded in `agents/internal/scaffold/assets/skills/migrating-fleet-context/SKILL.md` (deleted 2026-09-21).
...
2. **Target Discovery**: `agents ls` for the registered fleet, then `agents drift --json` for one repository or `agents drift --all --json` for the fleet. See §7.6 for the two output shapes. (All three commands are deleted; the fleet registry and the drift classifier went with them.)
...
7. **Verification Gate**: Executes `agents drift` (now deleted), `agents doctor` and repository test suites (`go test ./...`).
...
### 7.6 Fleet Mode and Output Shapes (retired, deleted 2026-09-21)

The deleted `agents drift --json` and the deleted `agents drift --all --json` did
not return the same JSON type, and a consumer written against one broke on the
other:

| Invocation (both deleted) | Returns |
|---|---|
| `agents drift --json` (deleted 2026-09-21) | a single `DriftReport` **object** |
| `agents drift --all --json` (deleted 2026-09-21) | an **array** of `DriftReport` |
...
All four phases below were executed. Phases 2–4 were then deleted 2026-09-21
(Amendment 3), so read this as the record of how the architecture was adopted
rather than as a plan. Phase 1 is the only one still in force.

1. **Phase 1: Asset Embedding & Scaffold Completion** (`internal/scaffold/assets/`):
   - Embed `recording-what-you-learn` (the `migrating-fleet-context` asset named
     here is deleted), `.agents/AGENTS.md` starter template, and `docs/` store READMEs.
   - Update `scaffold.Create` to populate full 4-store skeleton and skills.
2. **Phase 2: Drift Subsystem & Doctor Diagnostics** (`internal/drift/`, `cmd_drift.go`, `internal/doctor/`) — **implemented, then deleted 2026-09-21**:
   - Implement canonical digest catalog and `DriftReport` schema.
   - Implement the deleted `agents drift [--json]` CLI command.
   - Add granular `scaffold:*` doctor checks.
3. **Phase 3: Refined `agents update`** (`cmd_fleet.go`) — **implemented, then deleted 2026-09-21**:
...
4. **Phase 4: Model A Migration Skill Authoring** (`.agents/skills/migrating-fleet-context/`) — **implemented, then deleted 2026-09-21**:
