...
invalidate the recommendations above. Q1, Q2, Q3, Q8, Q9, and Q10 are
resolved; Q4–Q7 remain open. Until the queue is empty, §0 is provisional.
...
### 0.4 Q3 resolution (accepted 2026-09-18): no persisted profile
...
| V16 | `local_agents` | A v2 manifest is present in a repository whose `.agents/` is git-excluded or otherwise untracked. |
...
| Validation | V1–V16, including duplicate JSON keys, `..`, absolute paths, symlink components, `.agents/` stores, overlap, case-only collisions | A manifest that redirects a write outside the repo or onto another store |
...
| `--local` repo silently loses its manifest on clone | V16 rejects v2 in a repo whose `.agents/` is excluded; `init --local --template` refuses |
...
1. this design, its implementation plan, and its migration playbook are
   reviewed; the §0 decisions and the review queue Q1–Q10 are resolved or
   explicitly deferred;
